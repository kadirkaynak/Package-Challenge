...
Approach: The solution is to consider all subsets of items and calculate the total weight and value of all subsets.
Consider single subsets with a total weight less than W.
Select the maximum value subset from all such subsets.
If the total cost of the packed items is the same for the two item groups, select the combination of items with the lower total weight.

java -cp PackageChallenge-1.0-SNAPSHOT.jar com.kadirkaynak.PackageChallengeDemo "filepath"
